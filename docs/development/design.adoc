...
* **Automatisches Wiederanmelden:** Beim Start prüft der AuthProvider, ob ein gespeichertes Token im SecureStore liegt. Ist es noch gültig, bleibt die Sitzung aktiv, sonst erscheint direkt der Login-Screen.
...
=== Offline-Stand
* **Gespeicherte Tokens:** Derzeit schreibt nur `SecureStoreAuthRepository` die Authentifizierungsdaten verschlüsselt ins Gerät. Habit-Listen werden ausschließlich live vom Server geladen; Caching ist als zukünftige Erweiterung markiert.
...
* **Theming:** ThemedView/ThemedText nutzen presentation/styles/colors.ts, sodass alle Screens dieselben Farbwerte ziehen, UI-Änderungen (z. B. neue Palette oder Dark Mode) nur an einer Stelle angepasst werden müssen und keine Komponente eigene Farben pflegt.
...
| `/auth/register` | POST | `{ email, password, displayName }` | 201 + Login-Token + Nutzerprofil
| `/auth/login` | POST | `{ email, password }` | 200 + Login-Token + Nutzerprofil
| `/habits` | GET | Header `Authorization: Bearer <Token>` | Liste aller Habits inkl. Plan und Streak
| `/habits` | POST | `{ title, schedule, reminderTime }` | 201 + neu angelegtes Habit
| `/habits/{id}` | PATCH | Felder wie `{ title?, reminderTime?, isArchived? }` | 200 + aktualisiertes Habit
| `/habits/{id}` | DELETE | — | 204 (kein Inhalt)
| `/habits/{id}/entries` | POST | `{ date, status }` | 201 + aktualisierte Streak-Information
| `/profile` | GET | — | Aktuelles Profil und Statistiken
| `/profile` | PATCH | `{ displayName?, avatarUrl? }` | 200 + aktualisiertes Profil
| `/quotes/daily` | GET | — | Text des Tageszitats
|===
...
[mermaid]
....
include::../development/plantuml/Sequenzdiagramm_AutomatischeAnzeige_Tagesplan.md[lines=2..28]
....
