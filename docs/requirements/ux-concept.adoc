...
Das Produkt ist primär für Studierende in ihrem Alltag konzipiert um positive Gewohnheiten anzutrainieren oder negative Gewohnheiten abzugewöhnen, kann aber von prinzipiell allen Personengruppen genutzt werden.
...
Zudem könen sie persönliche Habits konfigurieren (z.B.: Festlegung der genauen Schrittzahl, die man pro Tag laufen möchte).
...
==== Ist Prinzipiell eine Verbindung zu weiteren Diensten erforderlich/vorgesehen?
Weitere Verbindungen könnten externe APIs wie Apple Fitness, Google Kalender, Apple Kalender, jegliche Fitness Apps (MCI) oder Produktivitätstools sein.
Diese Implementierungen sind erstmal nicht in diesem Projekt vorgesehen.
...
Es ist angedacht, dass ein Benutzer welcher in unserer App eingeloggt ist, über ein Profil verfpgt, welches er auf privat oder öffentlich stellen kann. Nutzer sollen untereinander die Erfolge anderer Akteure einsehen können, die auch ein öffentliches Profil haben. Unser Ansatz ist: Motivationssteigerung durch freiwilligen Wettkampf. Sollte ein Benutzer nicht wollen, dass andere seinen Stand einsehen können kann er dies verhindern, indem er sein Profil auf privat stellt.
...
* Im Dark Mode werden Kontraste bei Texten nicht richtig angezeigt 
* Teile der UI unübersichtlich da die momentan nur als Placeholder dienen
