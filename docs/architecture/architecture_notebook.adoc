...
* **Primäre Plattform:** iOS (React Native/Expo) +
* **Optional:** Browsernutzung und Android (nicht primär unterstützt) +

=== Schnittstellen- und Code-Dokumentation
...
* **Backend-Code-Doku (JSDoc):** Unter `docs/api/index.html` liegt die generierte HTML-Dokumentation der Express-Routen, Middleware und Utils. Regeneration bei Codeänderungen: im Verzeichnis `src/backend` `npx jsdoc -c jsdoc.json` ausführen.

=== Geplante externe Integrationen

Folgende Integrationen sind für zukünftige Versionen vorgesehen:

* Apple Fitness
* Google/Apple Kalender
* Wearables (z. B. HealthKit, Schrittzähler)
* Produktivitätstools
