...
*Backend-Container:* +
Für das Backend existieren ebenfalls Docker-Container (siehe `src/backend/Dockerfile` und `docker-compose.yml`). Diese ermöglichen einen containerisierten Betrieb des Backends und der Datenbank für lokale Entwicklung oder Deployment.

*Wichtiger Hinweis:* +
Im Rahmen dieses Projekts wird das Backend jedoch **nicht** im Container betrieben, sondern klassisch auf einem bereitgestellten lokalen Server ausgeführt. Die Container-Lösung für das Backend ist vorhanden und dessen Betrieb wird in der Betriebsdokumentation beschrieben.
