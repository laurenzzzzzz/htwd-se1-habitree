...
* **Portabilität**
** *Gemeinsame Code-Basis (Android & iOS):* Expo (React Native) ermöglicht die Pflege einer einzigen Codebasis für beide Plattformen.
** *Plattformunabhängige Schnittstellen:* REST-API kann von jeder Client-Plattform genutzt werden, nicht nur der App.
...
* **Skalierbarkeit**
** *Horizontale Skalierung:* Node.js-Backend kann containerisiert (z.B. via Docker) betrieben und skaliert werden.
** *Effiziente Datenbank:* PostgreSQL kann über Indexe, Abfragen und ggf. Caching skaliert werden.
...
* Die Zielgruppe sind vor allem Einzelpersonen, die persönliche Routinen tracken wollen, eine Mehrbenutzerfähigkeit (z.B. Teams oder Gruppenfunktionen) ist vorerst nicht vorgesehen.
...
* Benutzerverwaltung und Authentifizierung werden vollständig im eigenen System umgesetzt – keine externe Auth-Plattform wie Firebase wird verwendet.
* Das Backend wird auf einem selbstverwalteten Server betrieben – Cloud-native Dienste wie AWS, Firebase oder Azure werden nicht genutzt.
...
* Die geplanten Nutzerzahlen sind zunächst gering, was eine einfache Architektur und Serverinfrastruktur erlaubt.
...
* Für Statisiken die Grafen nutzen werden Java eigene Bibliotheken benutzt, da die implementierung für das Dev-Team zu aufwendig ist. 
