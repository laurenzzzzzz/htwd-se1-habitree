...
** *Health-Checks und Rollback:* Das System soll bei Inkonsistenzen in Datenbeständen o. ä. automatisch einen Rollback durchführen bzw. diesen beheben.
...
| Service Layer | Orchestriert Anwendungsabläufe mit Hilfe der Fachobjekte | [application/services/HabitService.ts], [application/services/AuthenticationService.ts], [context/AuthContext.tsx]
