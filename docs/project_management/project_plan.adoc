...
== Einführung
Der vorliegende Projektplan dokumentiert die organisatorischen, managementbezogene jedoch auch technische Praktiken des Projekts `Habitree` im Rahmen der Module Software Engineering 1 und Software Engineering 2.  +
Er beschreibt die Struktur und Rollenverteilung des Teams, die eingesetzten Management und Entwicklungsmethoden, sowie positive und negative Ereignisse mitsamt deren Ursachen und unseren entsprechenden learnings. Darüber hinaus enthält der Projektplan ergänzende Projektartefakte wie Definition of Ready und Definition of Done. +
 
Ziel des Dokuments ist es, die angewandten Vorgehensweisen transparent und nachvollziehbar zu machen. Hierbei beziehen wir uns primär auf organisatorische Abläufe, von der Sprintplanung über die Ausgestaltung des GitHub-Boards und der KPIs bis hin zum Umgang mit Risiken, Qualitätsmaßnahmen und dem Feature Stop am Projektende. +
Die beschriebenen Praktiken spiegeln sowohl theoretische Grundlagen des Scrum-Frameworks als auch deren Anpassung an unser Team im universitären Umfeld wieder.  +
Damit bietet der Projektplan eine umfassende Übersicht über Struktur, Ablauf und methodische Entscheidungen, die die Entwicklung von Habitree über die letzten 2 Semester geprägt haben.
...
- Weekly-Meetings (45 - 90 Minuten) zur Besprechung und Aufzeigung des aktuellen Arbeitsstatus der Mitglieder, Aufgabenverteilung, Identifikation von Hindernissen und operativer Planung für die nächste Woche +
...
Domänenwissen eignet er sich durch Bücher (wie z.B.: "Atomic Habits", "1% Methode" oder "The Power of Habits") und Internet an.  +
...
In unserem Team kümmert er maßgeblich sich um die Vision  sowie um die User Stories und dessen Planung.  +
**- Scrum Master:** +
...
- Eigende Meetings, in welchem es um technische Aspekte der Umsetzung geht
- Programmierung des Habit Trackers  +
...
- Zerlegung von hochpriorisierten User Stories inkleinere Tasks, vor allem in den späteren Iterationen.  +
...
- Jedes Teammitglied hat 10 min Zeit positive und negative Eindrücke und Ereignisse, die er während der letzten Iteration erfahren hat, darzulegen.  +
...
| Review von Ergebnissen | Am Ende jeder Iteration werden unsere bisherigen Ergebnisse Stakeholdern vorgestellt. Am Ende der zweiten Iteration wurden so z.B. unser Prototyp sowie Wireframes und Wireflows einer anderen Gruppe in einem Microsofft Teams- Meeting dargelegt. Wir holten uns über diesen Weg Feedback, Verbesserungsvorschläge und Ideen für weitere User Stories und Personas ein. Zudem wurde auch ein Meeting mit unseren Coach und den Professor durchgeführt um unseren bisherigen Arbeitsstand und Arbeitsphilosophie zu bewerten. Nach jedem Meeting mit weiteren Stakeholdern besprechen wir im Anschluss noch einmal kurz die Resultate, Protokollieren diese und leiten Aufgabe/Änderungen daran ab. User Storys, welche im Sprint nicht geschafft wurden, landen im Product Backlock.
...
Update/Delete/Add (Datei) {Beschreibungstext(Deutsch)} 
| Dokumentation
...
| Initiale Dokumentation von Risiken in separaten Git Risk Board. Hierbei werden die Risiken möglichst genau definiert.  +
- Das Risiko- Board wird bei Bedarf geupdatet und um neue Risiken ergänzt. Bestehende Risiken werden in regelmäßigen Abständen durchgangen und verwaltet hinsichtlich des Aufwandes und Einflusses.  +
- Auftretende Fehler und Risiken werden direkt in Meetings besprochen und nach Dringlichkeit priorisiert +
- Fehler und Bug-Fixes werden als eigene Tasks mit dem Label Defects verwaltet und im Github Project mit eigener Sicht dargestellt. +
| Entwicklungstechnologie
| Verwendung von Expo mit React Native zur plattformübergreifenden Entwicklung +
- Entwicklung in JavaScript und in TypeScript  +
- Nutzung der Expo Go App zur Live Vorschau auf echter Hardware +
- Ausführung der Entwicklungsumgebung in Docker Containern zur Sicherstellung konsistenter Abhängigkeiten und Vermeidung von Konfigurationsproblemen +
...
| CI/CD & Automatisierung (Continuous Integration/ Continuous Deployment)
| Nutzung von GitHub Actions zur automatisierten Qualitätssicherung +
- Trigger bei Pushes auf den Main-Branch zur sofortigen Überprüfung von Änderungen +
- Durchführung automatisierter Security-Scans mittels Shai-Hulud Detector +
- Prüfung der Projektabhängigkeiten und Umgebung durch Expo Doctor +
- Ausführung der Tests zur Gewährleistung der Funktionalität
...
- Es existieren ggf. automatisierte Unit Tests/Alle Tests werden überprüft. +
...
- Es entsteht Mehrarbeit für die verbleibenden Gruppenmitglieder, insbesonders die Entwickler  +
...
(3) Discord meist für Austausch von Ergebnissen, Quellen, Wireframes etc. während des Meetings +
(4) Miro- Board für Festlegung von Zufriedenheit nach Meetings und Templates (z.B.: für Git Commit- Messages)  +
...
- Man sollte sich nicht einlullen lassen und nicht immer Ausreden wie 'Das werde ich heute Abend noch machen...' akzeptieren. +
...
- Wenn User Stories oder andere Artefakte nicht abgeschlossen werden innerhalb eines Sprints, werden sie erst einmal in den Product Backlog zurückgeführt +
- Ggf. werden Stories die nicht abgearbeitet werden konnten in feinere Einheiten zerlegt am Sprintende (innerhalb von Refinement Meetings), allenfalls wird deren Komplexität aber vom Team erneut kritisch überprüft. +
...
| Stabilisierung, Qualitätssicherung und Risikovermeidung in der Abhschlussphase - Feature Stop am 09.01.2025
| - Einführung eines Feature- Stops am dem 09.01.2025 um sich in der  verbleidenden Projektzeit auf Qualitätssicherung, Feinschliff und Dokumentation auszurichten +
- Ab diesem Zeitpunkt werden keine neuen funktionalen Features mehr implementiert, stattdessen werden nur noch: +
(1) Kleiner UI- Anpassungen und Bugfixes vorgenommen +
(2) Bestehende Funktionen stabilisiert +
(3) Letzte Tests durchgeführt (welche auch bis zum 12.01.2025 abgeschlossen werden sollen) +
(4) Finalisierung  der Dokumentation und Reflexion +
- Ziele sind die Redution von Projektrisiken und Stress bei den Mitgliedern, sowie die Vermeidung von Implementierungsfehlern, und die sicherstellung, dass bei Abgabe und Präsentation der Anwendung alle bislang implementierten Dinge stabil und effizient funktionieren. +
|===
...
| Tests orientieren sich an den *FIRST-Prinzipien* (Fast, Independent, Repeatable, Self-Validating, Timely). +
- *Verifikation* (funktionale Korrektheit) und *Validierung* (Erfüllung der User Stories) +
- *White-Box-Tests*: Prüfung interner Logik mittels Code Coverage (Ziel: 60% für Branches, Functions, Lines, Statements) +
- *Black-Box-Tests*: Prüfung aus Nutzersicht anhand von User Stories und Akzeptanzkriterien +
- Testmethoden: Äquivalenzklassenanalyse, Grenzwertanalyse +
- Namenskonvention: `<Funktion>_<Szenario>_<ErwartetesErgebnis>` +
- Detaillierte Testdokumentation in `testing.adoc`

|===
| Aktueller Teststand (15.01.2026) | Kurzbeschreibung

| Coverage (Statements) | 97.98% - aktuell sehr gut; Ziel (>80%) übertroffen
| Coverage (Branches) | 92.21% - gute Verzweigungsabdeckung, noch kleine Ecken in Policies
| Coverage (Functions) | 100% - alle Funktionen sind getestet
| Coverage (Lines) | 99.24% - praktisch vollständige Zeilenabdeckung
| Tests (Anzahl) | 202 automatisierte Tests (Unit + Integration)
| Test-Suites | 15 Test-Suites (Unit + Integration)
| Systemtests | 10 manuelle System-/Smoke-Tests dokumentiert (ST-01..ST-10)
| Wichtige Verbesserungen | HabitService: von 41% → 100% Coverage; HabitSchedulePolicy: 53% → 94%; Habit Entity: 100% Statements
| Bleibende Lücken | NotificationService: noch ohne automatisierte Tests; einige Interfaces/Ports sind typbedingt nicht testbar (0% erwartet)
| Nächste Schritte (kurz) | 1) NotificationService-Tests (Mock INotificationPort)  2) CI-Check: Coverage-Gates einführen  3) Konsistente Kommentar-Header in allen Testdateien
|===

| Testpyramide & Testarten
| *Teststufen:* +
(1) Unit-Tests zur Überprüfung einzelner Methoden +
(2) Integrationstests für die Zusammenarbeit mehrerer Units +
(3) Systemtests für das gesamte System +
*Unterteilung:* Positivtests (valider Input) und Negativtests (Exception-Handling)

| Test-Framework & Werkzeuge
| *Framework:* Jest mit `jest-expo` Preset für React Native/Expo +
*Mocking:* Repository-Interfaces mit `jest.fn()`, eigene Mock-Implementierungen (z.B. `MockHabitsRepository`) +
*Coverage:* Automatische Messung für `domain/**` und `application/**` Layer, Report via `npm run test:coverage`

| Automatisierung & CI/CD
| *GitHub Actions Workflow* bei Push auf `main`: +
(1) Code auschecken → (2) Node.js 20.x installieren → (3) `npm ci` → (4) Security Scan → (5) Expo Doctor → (6) `npm test` +
*Lokale Befehle:* `npm test`, `npm run test:watch`, `npm run test:coverage`

| Testorganisation & Struktur
| *Ordnerstruktur:* `__tests__/domain/` (Unit), `__tests__/integration/` (Integration), `__tests__/system/` (Manuelle Systemtests) +
*Dateikonvention:* `*.test.ts` / `*.test.tsx`, eine Datei pro Entity/Service +
*Aufbau:* `describe()` für Funktionen, `beforeEach()` für Reset, `it()`/`test()` für Testfälle

| Integration ins GitHub-Board
| Bugs werden als Issues mit Label `defect` erfasst und erscheinen automatisch im separaten Defect-Board. +
Priorisierung nach Risiko, Komplexität und Geschäftswert.

| Integration in den Scrum-Zyklus
| Sprint Reviews enthalten Akzeptanztests zu ausgewählten Stories/Features. +
Statische Tests in Weekly-Meetings durch PO. +
Vorab-Absprache der Testevaluationskriterien mit Scrum Master, PO und Entwickler.

| Manuelle & Explorative Tests
| *Systemtests:* Dokumentiert in `__tests__/system/SYSTEMTESTS.md` (ST-01 bis ST-04: Login, CRUD, Streak, Validierungen) +
*Explorative Tests:* In Weekly-Meetings mit PO, Fokus auf Usability +
*Akzeptanztests:* In Sprint Reviews mit externem Team (z.B. Team 1C)
|===
