...
|- Unsere Tests orientieren sich an den `First`- Regeln <???> +
- Habitree muss hierbei verifiziert und validiert werden.
- Unsere Tests untergliedern sich in mehrere Arten: +
(1) White- Box- Tests: Messung mittels der KPI `Code Coverage`:
- Ziel ist es die interne Logik und Kontrollflüsse des Codes basierend auf Wissen über die Implementierung zu prüfen. +
(2) Black Box Tests: +
- Ziel hierbei ist es zu überprüfen, ob die Anwendung sich aus Sicht der Nutzer so verhält, wie es in den User Stories und Akzeptanzkriterien geschrieben wurde. Der Interne Code wird hierbei als unbekannt gesehen. +
- Diese Tests erfolgen explorativ (z.B.: manuelles durchklicken der App zur gewünschten Spezifikation von User Story X) +
(2.1) Äquivalenzklassenanalyse +
(2.2) Grenzwertanalyse +
- Die Tests haben folgende Namenskonventionen: <???> +
- Es wurde eine passende Ordner Struktur für das File- Layout angelegt. Der konkrete Aufbau befindet sich im Dokument `testing.adoc`. +
- Die Tests erfolgen schon während des jeweiligen Sprints, spätestens bei Überprüfung der Kriterien der Definition of Done für die enstprechenden Stories. Eine User Story darf erst auf Done gestellt werden, wenn diese die Kriterien der DoD erfüllt, was bei uns bedeutet, dass hinreichend viele Tests durchgeführt wurden. +
- Testoptimierung wird bei uns jeweils nach dem Sprint in der Retrospektive besprochen. +
...
Dokument anlegen: testing.adoc mit Test- Ordnerstruktur +
Namenskonventionen für tests nieder schreiben +

Mocking vorhanden ???


Welche Qualitätseigenschaften testes du am meisten? Folie 28 +


Automatisiertes vs Exploratives Testen was machen wir genau? +
- Automatisiertes Testen: Machen wir bei Push/Pull Requests und den Code auf Git Hub Pushen doer Pullen mit Git Hub Actions +
- Exploratives: Unit Tests +


Konkrete Umsetzung der White Box tests kurz hier niederschreiben und genauer noch mal in Testing.adoc ergänzen +

Testorganisation (Seite 24- 30) muss erklärt werden. +

Beispiele bei First Regeln nennen +

Fragen aus Vorlesung von Prof. Anke beantworten- ggf. im Q&A