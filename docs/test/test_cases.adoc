...
Aktueller Stand (aktualisiert):
- Statements: 77.57%
- Branches: 68.70%
- Functions: 79.31%
- Lines: 79.66%

In den letzten Iterationen haben wir die Coverage deutlich erhöht (+~20%) durch folgende Ergänzungen:
- Neu implementierte Domain-Entity Tests:
  - `Streak.entity.test.ts`
  - `Achievement.entity.test.ts`
  - `TreeGrowth.entity.test.ts`
- Neu implementierte Service-/Integration-Tests:
  - `StreakService.integration.test.ts`
  - `AchievementService.integration.test.ts`
  - `TreeGrowthService.integration.test.ts`
  - `QuoteService.test.ts` (erweitert, inkl. `fetchQuoteOfDay`)
  - `HabitService.integration.test.ts` (MockHabitsRepository, E2E-Flow)

Stark abgedeckt sind nun insbesondere die Domain-Entities (Habit, User, Quote, Streak, Achievement, TreeGrowth) sowie die genannten Services.

Verbleibende Lücken / Fokusbereiche (Priorität):
- `NotificationService` — noch ohne Tests (Port/Adapter-Interaktion testen)
- Weitere Branch-Cases in `HabitService` (Payload-Varianten: weekDays / intervalDays / optional fields)
- Optional: zusätzliche Edge-Case-Tests für `QuoteService` (Statistik-/Random-Pflege)
- Interfaces/Ports (domain/ports, domain/repositories) zeigen 0% Coverage — erwartbar (keine Implementierungslogik)
...
4. TESTING_README.md um ein kurzes Mock-Klassen-Template ergänzen.
